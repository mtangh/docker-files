...
#S:CATALINAOUTROTATION
ROTATELOGS="`which rotatelogs 2>/dev/null`"
if [ -z "$ROTATELOGS" ] ; then
  for dir in /usr/sbin /sbin /usr/bin /bin /usr/local/sbin /usr/local/bin ; do
    [ -x "$dir/rotatelogs" ] &&
      ROTATELOGS="$dir/rotatelogs"
  done
fi
#E:CATALINAOUTROTATION
...
#S:CATALINAOUTROTATION
  if [ -x "$ROTATELOGS" ] ; then
    echo "Using ROTATELOGS:      $ROTATELOGS"
  fi
#E:CATALINAOUTROTATION
fi
...
#S:CATALINAOUTROTATION
    ...
    "$_RUNJAVA" "$LOGGING_CONFIG" $JAVA_OPTS $CATALINA_OPTS \
      ...
      -Djava.io.tmpdir="$CATALINA_TMPDIR" \
      org.apache.catalina.startup.Bootstrap "$@" start \
      2>&1 | "$ROTATELOGS" "$CATALINA_OUT".%Y%m%d 86400 540 &
    fi
#E:CATALINAOUTROTATION
...
#S:CATALINAOUTROTATION
      ...
      grep -- " -Dcatalina.home=${CATALINA_HOME} " |
      grep -v grep |
      awk '{print($2);}' > "$CATALINA_PID"
    fi
#E:CATALINAOUTROTATION
  fi
